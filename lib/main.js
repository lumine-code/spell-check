...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "spell-check",
      tips: [
        "You can open spelling corrections for the word under the cursor with {{ 'spell-check:correct-misspelling' | keystroke }}",
        "You can spell-check a selection in any file, whatever its language, with the check selected command.",
      ],
    };
  },
  ...
  activate() {
    ...
    this.subs.add(
      lumine.commands.add("lumine-workspace", {
        "spell-check:toggle": {
          description: "Turn spell-checking on or off for this file's grammar.",
        ...
        "spell-check:correct-misspelling": {
          description: "Offer corrections for the misspelling under the cursor.",
          didDispatch: (event) => this.correctMisspelling(event),
        },
        "spell-check:add-known-word": {
          description: "Add the word under the cursor to the known-words list.",
          didDispatch: (event) => this.addKnownWord(event),
        },
        "spell-check:check-selected": {
          description: "Spell-check only what is selected, ignoring the rest.",
          didDispatch: (event) => this.checkSelected(event),
        },
        "spell-check:clear-checked-selection": {
          description: "Drop the messages from the last selection check.",
          didDispatch: () => {
            this.indie?.clearMessages();
            this.forgetCheckedSelection();
          },
        },
      }),
    );
  ...
  // The corrections, as autocomplete suggestions.
  //
  // The list, its keyboard handling, its theming and its placement are the
  // autocomplete package's, which ships with the editor — so `spell-check` draws
  // no picker of its own. Three fields make a completion menu behave as a
  // correction menu:
  //
  // * `ranges` replaces the misspelled word wherever the cursor sits inside it,
  //   instead of the typed prefix ending at the cursor;
  // * high priorities put corrections above the buffer's own words, and keep
  //   them from being dropped by a provider that excludes lower priorities;
  // * `activatedManually` limits them to a menu the user asked for. Typing a
  //   word raises the same `triggerKind`, so nothing else can tell the two
  //   apart, and corrections offered on every keystroke of a prose file would
  //   be noise.
  //
  // Autocomplete confirms a single suggestion in manual mode without showing a
  // list, so a misspelling with exactly one correction is simply fixed.
  provideAutocomplete() {
    return {
      // Corrections follow the linter, which reaches past the workspace center
      // to every editor registered through `linter.editors` — the commit box.
      // Those editors are watched under the `default` label, and a provider
      // without labels serves the center only.
      labels: ["workspace-center", "default"],
      scopeSelector: "*",
      inclusionPriority: 100,
      suggestionPriority: 100,
      // `ranges` suggestions are exempt from prefix filtering anyway: what the
      // user typed says nothing about which correction applies.
      filterSuggestions: false,
      getSuggestions: ({ editor, bufferPosition, activatedManually }) => {
        if (!activatedManually) return [];
        const found = this.checkers.get(editor)?.correctionsAt(bufferPosition);
        if (!found) return [];

        return found.corrections
          .filter((correction) => correction.isSuggestion)
          .map((correction) => ({
            text: correction.suggestion,
            ranges: [found.range],
            type: "value",
            iconHTML: '<i class="icon-book"></i>',
            description: `Correct “${editor.getTextInBufferRange(found.range)}”`,
          }));
      },
    };
  },
  ...
  // The dedicated keystroke. It opens the autocomplete menu, which this package
  // supplies the corrections to — so there is one list in the editor rather than
  // one per package that has replacements to offer.
  //
  // Nothing is done when the cursor is not in a misspelling: the menu would open
  // on the buffer's own words, which is not what the command was asked for.
  correctMisspelling(event) {
    const editor =
      lumine.workspace.getTextEditorForElement(event?.target) ??
  ...
  // Adds the word under the cursor to Known Words. A correction replaces text
  // and belongs in the corrections menu; this changes a setting, so it is a
  // command of its own rather than a suggestion that inserts the word it
  // already found there.
  //
  // Deliberately independent of `addKnownWords`, which decides whether the same
  // action is offered among the code actions at the cursor. A command named "add
  // known word" is the user asking outright and needs no setting's permission.
  // The range comes from the cached lint result rather than from
  // `correctionsAt`, so no dictionary suggestions are computed to be discarded.
  addKnownWord(event) {
    const editor =
    ...
    const range = this.checkers.get(editor)?.misspellingAt(editor.getCursorBufferPosition());
    if (!range) {
      lumine.notifications.addWarning("No misspelling under the cursor.");
      return;
    }

    this.getInstance(this.globalArgs).addKnownWord(editor.getTextInBufferRange(range));
  },
  ...
  async checkSelected(event) {
    ...
    // The linter shows them; the checker is what every corrections surface asks,
    // so it has to know about them too. One check stands at a time, matching
    // `setAllMessages`, so the previous one is dropped rather than accumulated.
    this.forgetCheckedSelection();
    this.checkedSelectionEditor = editor;
    this.checkerFor(editor)?.setSelectionMessages(messages);
  ...
  // Drops the corrections for the last checked selection. The linter's own copy
  // is cleared by whoever calls this.
  forgetCheckedSelection() {
    const previous = this.checkedSelectionEditor;
    this.checkedSelectionEditor = null;
    if (previous) {
      this.checkers.get(previous)?.setSelectionMessages([]);
    }
  },
