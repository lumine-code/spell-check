...
module.exports = {
  activate() {
    ...
    for (const setting of ["grammars", "excludedScopes", "severity", "checkMiniEditors"]) {
      this.subs.add(
  ...
  provideLinter() {
    ...
    return {
      ...
      grammarScopes: ["*"],
      lint: (editor) => {
        if (!this.shouldCheck(editor)) return [];
  ...
  // A search field, a picker's input, a rename dialog: all editors, and all
  // without a grammar, so the plain-text entry in `grammars` matches them and the
  // command palette ends up underlined in red. Nobody writes prose in one, so
  // they are off unless asked for.
  //
  // Asked on every lint rather than when the checker is built, so turning the
  // setting off takes effect on the next run instead of outliving it in a cache.
  shouldCheck(editor) {
    return !editor.isMini() || lumine.config.get("spell-check.checkMiniEditors");
  },
