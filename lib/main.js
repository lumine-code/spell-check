...
module.exports = {
  ...
  addKnownWord(event) {
    ...
    // Two different refusals, and saying which is the difference between a user
    // moving the cursor and a user turning on a setting.
    const target = found.corrections.find((correction) => !correction.isSuggestion);
    if (!target) {
      lumine.notifications.addWarning("Known Words actions are turned off.", {
        description:
          "Enable **Offer Known Word Actions** in the Spell Check settings to add words.",
      });
      return;
    }

    this.applyCorrection(editor, found.range, target);
  },
