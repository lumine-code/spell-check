...
module.exports = {
  ...
  provideLinter() {
    ...
    return {
      ...
      // arrives here and the checker decides.
      grammarScopes: ["*"],
      // Which editors to be asked about. By default only the documents somebody
      // opened: a package builds editors of its own to render a diff with, or a
      // patch preview, or a commit box in a dock, or the field inside a picker,
      // and none of them carries a grammar — so the plain-text entry in
      // `grammars` reached every one and the command palette came up underlined
      // in red. A buffer nobody has saved yet still counts: it is a pane item,
      // it simply has no path.
      //
      // A getter because the linter reads this on every run, which is what lets
      // the setting take effect without re-registering the provider. The setting
      // says "all" where the service says "any"; the service word is about which
      // editors it may ask about, and "all of them" is the plainer thing to read
      // in a settings pane.
      get editors() {
        return lumine.config.get("spell-check.editors") === "all" ? "any" : "center";
      },
      lint: (editor) => {
