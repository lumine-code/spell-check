...
// Adds a checker's range to an integer set.
//
// A checker reports half-open ranges — `text.slice(start, end)` is the word it
// means — while a `MultiRange` is a set of integers, so the exclusive end has to
// come off. Handing it over as it stands puts the character *after* the word in
// the set, and two words either side of a single character then merge into one:
// `openDatabase` and `databasePath` around a `(` were reported as a misspelled
// `openDatabase(databasePath`, since `addMisspellings` only splits on
// whitespace.
function appendHalfOpenRange(multiRange, { start, end }) {
  if (end > start) multiRange.appendRange(start, end - 1);
}
...
class SpellCheckerManager {
  ...
  async gatherRanges(args, text) {
    ...
    for (let results of allResults) {
      ...
      if (!results.invertIncorrectAsCorrect && results.correct) {
        for (range of results.correct) {
          appendHalfOpenRange(correct, range);
        }
      }
      ...
      if (results.incorrect) {
        ...
        for (range of results.incorrect) {
          appendHalfOpenRange(newIncorrect, range);
        }
      }
    }
    ...
    if (incorrects.length === 0) {
      return { correct: correct.getRanges(), incorrect: [] };
    }
    ...
    return { correct: correct.getRanges(), incorrect: intersection.getRanges() };
  }
