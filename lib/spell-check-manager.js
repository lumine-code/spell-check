...
// Collects a checker's range as an inclusive pair.
//
// A checker reports half-open ranges — `text.slice(start, end)` is the word it
// means — while an integer set holds the integers themselves, so the exclusive
// end has to come off. Handing it over as it stands puts the character *after*
// the word in the set, and two words either side of a single character then
// merge into one: `openDatabase` and `databasePath` around a `(` were reported
// as a misspelled `openDatabase(databasePath`, since `addMisspellings` only
// splits on whitespace.
//
// The pairs are collected raw and normalized once per set, rather than merged
// on every append: normalizing is a sort, and doing it per word would put a
// sort inside the loop that runs once per misspelling in the buffer.
function appendHalfOpenRange(pairs, { start, end }) {
  if (end > start) pairs.push([start, end - 1]);
}

// Subtracts `b` from `a`, where both are ascending arrays of disjoint inclusive
// `[start, end]` pairs.
//
// The library's own `subtract` re-scans its range array from the end for every
...
class SpellCheckerManager {
  ...
  async check(args, text) {
    ...
    const { correct, incorrect } = await this.gatherRanges(args, text);
    ...
    // incorrect providers.
    let ranges = incorrect;
    if (correct.length > 0) {
      ranges = await this.confirmRemainders(args, text, incorrect, correct);
      if (ranges.length === 0) {
        this.log("no spelling after the correct ranges");
        return { misspellings: [] };
      }
    }

    // Back to the half-open ranges the rest of this method works in: an
    // integer set is inclusive at both ends.
    ranges = ranges.map(([start, end]) => [start, end + 1]);
  ...
  /**
   * Asks every enabled checker about `text` and reduces the answers to two
   * lists of inclusive index ranges, each sorted and disjoint: what some
   * checker confirmed correct, and what *every* checker that reports
   * misspellings agrees is one.
   * @param {Object} args
   * @param {string} text
   * @returns {Promise<Object>} `{ correct, incorrect }`.
   */
  async gatherRanges(args, text) {
    // We need a couple packages but we want to lazy load them to
    // reduce load time.
    // ESM-only from v6, and this method is already async, so import it here
    // rather than reaching for `require`.
    const { normalize, intersect } = await import("multi-integer-range");
    ...
    for (let results of allResults) {
      ...
      if (!results.invertIncorrectAsCorrect && results.correct) {
        for (range of results.correct) {
          appendHalfOpenRange(correctPairs, range);
        }
      }

      if (results.incorrect) {
        const newIncorrectPairs = [];

        for (range of results.incorrect) {
          appendHalfOpenRange(newIncorrectPairs, range);
        }

        incorrects.push(normalize(newIncorrectPairs));
      }
    }

    if (this.log.enabled) {
      this.log("merged correct ranges", correctPairs);
      this.log("merged incorrect ranges", incorrects);
    }

    if (incorrects.length === 0) {
      return { correct: normalize(correctPairs), incorrect: [] };
    }
    ...
    for (let incorrect of incorrects) {
      intersection = intersection === null ? incorrect : intersect(intersection, incorrect);
    }

    return { correct: normalize(correctPairs), incorrect: intersection };
  }

  /**
   * Subtracts the confirmed correct ranges, then asks again about whatever that
   * left behind.
   *
   * The checkers do not agree on where a word ends. Hunspell reports each run of
   * letters, so `graviss-sofistik` comes back as two misspellings, while the
   * Windows spelling service reports the whole of it as one — and the known-words
   * checker marks the parts it knows, because a dictionary flags a compound one
   * part at a time and the two would not line up otherwise. Subtracting the parts
   * from the whole then leaves text that no checker ever judged: the hyphen
   * between two known words, reported as "- is not in the dictionary", or
   * `.source` where only `graviss` is known, reported although `source` is a
   * word.
   *
   * A range the subtraction left untouched is already an answer and stands. Only
   * the remainders go back to the checkers — in one pass, and without the correct
   * ranges, which have had their say — and only what is flagged a second time is
   * reported, at whatever narrower range that pass gives it.
   * @param {Object} args
   * @param {string} text
   * @param {Array} incorrect - Inclusive ranges, sorted and disjoint.
   * @param {Array} correct - Inclusive ranges, sorted and disjoint.
   * @returns {Promise<Array>} Inclusive ranges, sorted and disjoint.
   */
  async confirmRemainders(args, text, incorrect, correct) {
    const flaggedAsIs = new Set(incorrect.map(([start, end]) => `${start},${end}`));
    const kept = [];
    const segments = [];
    let probe = "";

    for (const remainder of differenceOfSortedRanges(incorrect, correct)) {
      if (flaggedAsIs.has(`${remainder[0]},${remainder[1]}`)) {
        kept.push(remainder);
        continue;
      }

      // One text rather than one call for each remainder: a checker is charged
      // for the call about as much as for the words in it. A newline separates
      // them, since no checker reads a word across one.
      const fragment = text.substring(remainder[0], remainder[1] + 1);
      segments.push({ probeStart: probe.length, textStart: remainder[0], length: fragment.length });
      probe += fragment + "\n";
    }
    ...
    const { incorrect: reflagged } = await this.gatherRanges(args, probe);
    if (this.log.enabled) {
      this.log("re-checked remainders", probe, reflagged);
    }

    let segment = 0;
    for (const [start, end] of reflagged) {
      while (segment + 1 < segments.length && start >= segments[segment + 1].probeStart) {
        segment++;
      }

      const { probeStart, textStart, length } = segments[segment];
      const from = Math.max(start, probeStart);
      const to = Math.min(end, probeStart + length - 1);
      if (to < from) continue;

      kept.push([textStart + (from - probeStart), textStart + (to - probeStart)]);
    }
  ...
  addMisspellings(misspellings, row, range, lineBeginIndex, text) {
    // Nothing to mark. A range clipped to an empty line spans no text at all,
    // and an empty range would be reported as a misspelling of nothing.
