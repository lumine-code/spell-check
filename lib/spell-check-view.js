...
module.exports = class SpellCheckView {
  constructor(editor, spellCheckModule, manager) {
    ...
    this.correctMisspellingCommand = lumine.commands.add(
      lumine.views.getView(this.editor),
    ...
    editorElement.addEventListener("contextmenu", this.addContextMenuEntries);
    this.disposables.add(
      new Disposable(() =>
        editorElement.removeEventListener("contextmenu", this.addContextMenuEntries),
      ),
    );
    ...
    this.disposables.add(
      lumine.config.onDidChange("spell-check.grammars", () => {
        return this.subscribeToBuffer();
      }),
    );

    this.disposables.add(
      lumine.config.observe("spell-check.excludedScopes", (excludedScopes) => {
  ...
  subscribeToBuffer() {
    ...
    if (this.spellCheckCurrentGrammar()) {
      ...
      this.bufferDisposable = new CompositeDisposable(
        ...
        // The first check runs before the grammar has tokenized anything, so a
        // scope-narrowed configuration has no descriptors to weigh yet and
        // filters out every misspelling. Tokenizing fires once per grammar
  ...
  // Empties the layer without replacing it. Tearing the layer and its
  // decoration down and building both again on every update made the editor
  // re-derive every highlight from scratch, and handed any consumer holding the
  // old layer a destroyed object. `clear` emits one update for the whole batch.
  clearMarkers() {
    return this.markerLayer.clear();
  }

  addMarkers(misspellings) {
    // Under the hood, we spell-check the entire document; but we might end up
    // ignoring some of the misspellings based on their scope descriptor.
    //
    // Resolving a scope descriptor means walking the syntax tree, which costs
    // far more than marking the range does — over a second for a large Markdown
    // buffer. When nothing narrows the checked region and nothing is excluded,
    // no descriptor can change the outcome, so we never ask for one.
    const filtering = Array.isArray(this.scopesToSpellCheck) || this.excludedScopes.length > 0;

    for (let misspelling of misspellings) {
      if (filtering) {
        const scope = this.editor.scopeDescriptorForBufferPosition(misspelling[0]);
        if (this.scopeIsExcluded(scope)) continue;
      }
  ...
  getCorrections(marker) {
    ...
    if (this.buffer != null && this.buffer.file && this.buffer.file.path) {
      [projectPath, relativePath] = lumine.project.relativizePath(this.buffer.file.path);
    }
  ...
  addContextMenuEntries(mouseEvent) {
    ...
    const currentScreenPosition = lumine.views
      .getView(this.editor)
    ...
    if (
      ...
      if (corrections.length > 0) {
        this.spellCheckModule.contextMenuEntries.push({
          menuItem: lumine.contextMenu.add({
            "lumine-text-editor": [{ type: "separator" }],
          }),
        });
        ...
        for (let correction of corrections) {
          ...
          contextMenuEntry.command = ((correction) => {
            return lumine.commands.add(lumine.views.getView(this.editor), commandName, () => {
          ...
          contextMenuEntry.menuItem = lumine.contextMenu.add({
            "lumine-text-editor": [{ label: correction.label, command: commandName }],
          });
  ...
  makeCorrection(correction, marker) {
    if (correction.isSuggestion) {
      ...
      if (this.editor.buffer != null && this.editor.buffer.file && this.editor.buffer.file.path) {
        [projectPath, relativePath] = lumine.project.relativizePath(this.editor.buffer.file.path);
      }
