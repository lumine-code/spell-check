...
function segmentsMatch(descriptorSegment, selectorSegment, { enforceSegmentOrder = false } = {}) {
  ...
  // Scope-segment order has historically been treated as insignificant:
  // `.function.entity` and `.entity.function` are equivalent by default.
  //
...
class ScopeSelector {
  ...
  matches(scopeDescriptorOrArray, rawOptions = {}) {
    let options = {
      ...
      // `.entity.function`. Defaults to `false` to match prevailing editor
      // behavior.
...
//
// A subset of the established TextMate-compatible scope selector syntax is
// supported:
//
