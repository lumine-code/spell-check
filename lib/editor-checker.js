...
module.exports = class EditorChecker {
  ...
  //
  // A language mode that has not parsed does not refuse a scope descriptor —
  ...
  // span in the buffer as prose. Nothing re-lints when parsing catches up,
  // so those messages would stand until the next edit.
  whenParsed() {
    const settled = this.parsingOf(this.editor.getBuffer().getLanguageMode());
    ...
    return new Promise((resolve) => {
      ...
      // nothing left to scope: neither will ever finish parsing, and both end the
  ...
  parsingOf(languageMode) {
    // A Tree-sitter mode settles when its transaction does, which covers the
    // initial parse, injected layers and a re-parse still in flight after an
    // edit. The parserless null mode has nothing to wait for.
    return languageMode.atTransactionEnd?.() ?? null;
  }
