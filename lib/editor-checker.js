...
module.exports = class EditorChecker {
   ...
   * @param {AbortSignal} [options.signal] - Cancels a pending grammar wait.
  ...
  async lint({ signal } = {}) {
    ...
    if (misspellings.length > 0 && this.filtersByScope()) {
      // Before parsing, scope lookup returns only the root scope. Waiting here
      // keeps code in GFM fences and injected languages out of prose results.
      const settled = await this.editor.whenGrammarSettled({ signal });
      // A false result means the parse cannot safely answer the scope question:
      // the editor or grammar changed, the request was aborted, or parsing
      // failed. Null leaves the previous messages alone; the linter clears them
      // with the editor or schedules a fresh check for the new buffer state.
      if (!settled || this.editor.isDestroyed()) return null;
    }
